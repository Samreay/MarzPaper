...
class ThresholdFitter(Fitted):
    def __init__(self):
        self.params = [('sub', 0, 10, 'sub'), ('pow', 0, 5, 'pow'), ('x4', 0, 100, 'x_4'), ('x3', 0, 100, 'x_3'), ('x2', 0, 100, 'x_2'), ('x1',0,100, 'x_1'), ('alpha',0,100,'$\\alpha$'),('beta',0,4,'$\\beta$')]
        self.res = self.loadData()
        self.qops = [4,3,2,1]
        self.colours = ["#4CAF50", "#2196F3", "#E5A9A5", "#E53935", "#673AB7"]
        self.qopArr = [res[res[:,0] == p] for p in qops]
    ...
    def getChi2(self, params):
        ...
        for i, p in enumerate(params):
            if p < allParams[i][1] or p > allParams[i][2]:
                return None
        data = [(x[:,1]-params[0]) * (x[:,1]/x[:,2])**params[1] + ((x[:,1]/x[:,2])*params[6])**params[7] for x in qopArr]        
...
if __name__ == '__main__':
    ...
    cambMCMCManager.configureMCMC(numCalibrations=9,calibrationLength=500, thinning=5, maxSteps=30000)
