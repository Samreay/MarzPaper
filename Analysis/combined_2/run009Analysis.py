...
marzDir = os.path.join(directory, 'marz_chris')
marzDir = os.path.join(directory, 'marz_update2')
...
def loadData():
        ...
        with open(os.path.join(runzDir, filename), 'r') as f:
            ...
            for line in f:                
                ...
                if objects.get(key):
                    ...
                    objects[key] = {'RZ': float(i[runzZ]), 'QOP': int(i[runzQOP]), 'RXZ': float(i[runzXZ]) , 'REZ': float(i[runzEZ]), 'COR': helio/3.0e5}
            ...
            with open(os.path.join(marzDir, filename), 'r') as f:
                for line in f:
                    ...
                    if objects.get(idd):
                        ...
                        objects[idd] = {'MZ': float(i[marzZ]), 'MTID': i[marzTID].strip(), 'TYPE': i[marzT].strip(), 'MID': i[marzID].strip(), 'MQOP': int(i[marzQOP].strip())}
    ...
    for item in objects:
        ...
        if i.get('MZ') is not None and i.get('RZ') is not None and i.get('RXZ') is not None:
            results.append([i.get('MQOP'), i.get('AZ'), i.get('RZ'), i.get('MZ'), i.get('RXZ'), i.get('REZ'), i.get('COR'), i.get('MTID')])
...
def plotOffset(res, names):
    ...
    ax.text(-0.00071, 540, r"$\bar{\Delta z}\pm\sigma$", fontsize=16)
    ax.text(-0.00075, 500, r"$\mathrm{Autoz}$", fontsize=16, horizontalalignment='right')
    ax.text(-0.00075, 470, r"$\mathrm{Marz}$", fontsize=16, horizontalalignment='right')
    dd = 5
    ax.text(-0.00071, 500, r"$(%0.f \pm %0.f)\times 10^{-%d} $" % (np.mean(ad)*10**dd, np.std(ad)*10**dd, dd), fontsize=16)
    ax.text(-0.00071, 470, r"$(%0.f \pm %0.f)\times 10^{-%d} $" % (np.mean(md)*10**dd, np.std(md)*10**dd, dd), fontsize=16)

    
    ax.text(-0.00071, 400, r"$\tilde{\Delta z}$", fontsize=16)
    ax.text(-0.00075, 370, r"$\mathrm{Autoz}$", fontsize=16, horizontalalignment='right')
    ax.text(-0.00075, 340, r"$\mathrm{Marz}$", fontsize=16, horizontalalignment='right')
    d1 = 5
    d2 = 6
    ax.text(-0.00071, 370, r"$%0.f\times 10^{-%d} $" % (np.median(ad)*10**d1, d1), fontsize=16)
    ax.text(-0.00071, 340, r"$%0.f\times 10^{-%d} $" % (np.median(md)*10**d2, d2), fontsize=16)
    
    fig.savefig("systematic.png", bbox_inches='tight', dpi=600, transparent=True)
    fig.savefig("systematic.pdf", bbox_inches='tight', transparent=True)
